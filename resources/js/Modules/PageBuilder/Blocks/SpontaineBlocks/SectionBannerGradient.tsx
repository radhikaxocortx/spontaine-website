...
export interface SectionBannerGradientBlock extends BlockConfiguration {
  id?: number
  title: TextData
  description: ItemListField<TextData>
}

interface SectionBannerGradientProps {
  editMode?: boolean
  onFieldEdit?: (field: BlocKFieldInfo) => void
  blockData?: SectionBannerGradientBlock
  language?: Language
}

export const sectionBannerGradientBlock: SectionBannerGradientBlock = {
  title: {
...
export default function SectionBannerGradient({
  editMode = false,
  onFieldEdit,
  blockData = sectionBannerGradientBlock,
  language = 'en',
}: SectionBannerGradientProps) {
  ...
  return (
    <section
      ref={sectionRef}
      className={cn(
        'relative flex w-full flex-col items-center justify-center pb-20 pt-40 text-black',
            ...
            >
              {blockData?.description?.items.map((item) => (
                <p
                  ...
                  className='text-spontaine-dark font-body mx-auto max-w-[560px] text-[16px] font-normal leading-[1.8] sm:text-[20px]'
                >
                  <Localization
                    text={item.item}
                    language={language}
