...
export interface SectionBannerCleanBlock extends BlockConfiguration {
  id?: number
  title: TextData
  description: ItemListField<TextData>
}

interface SectionBannerCleanProps {
  editMode?: boolean
  onFieldEdit?: (field: BlocKFieldInfo) => void
  blockData?: SectionBannerCleanBlock
  language?: Language
}

export const sectionBannerCleanBlock: SectionBannerCleanBlock = {
  title: {
...
export default function SectionBannerClean({
  editMode = false,
  onFieldEdit,
  blockData = sectionBannerCleanBlock,
  language = 'en',
}: SectionBannerCleanProps) {
            ...
            >
              {blockData?.description?.items.map((item) => (
                <p
                  ...
                  className='font-body mx-auto max-w-[560px] text-[16px] font-normal leading-[1.8] text-gray-800 sm:text-[20px]'
                >
                  <Localization
                    text={item.item}
                    language={language}
